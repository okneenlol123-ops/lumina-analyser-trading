# main.py
# Final: Offline-Finanz-App — Extended, fixes for Rebalancing/Simulation/Statistiken/Wissensbasis
# Includes theme selection (Dark Black, Midnight Blue, Neon Green)
...
# default apply (will update after settings load)
...
profile_settings = load_json(profile_file(active_profile, "settings"), {})
profile_portfolio = load_json(profile_file(active_profile, "portfolio"), [])
profile_history = load_json(profile_file(active_profile, "history"), [])
profile_notifications = load_json(profile_file(active_profile, "notifications"), [])
profile_watchlist = load_json(profile_file(active_profile, "watchlist"), [])

# session defaults
if "profile" not in st.session_state:
    st.session_state.profile = active_profile
    st.session_state.settings = profile_settings
    st.session_state.portfolio = profile_portfolio
    st.session_state.history = profile_history
    st.session_state.notifications = profile_notifications
    st.session_state.watchlist = profile_watchlist
...
# -------------------------
# Authentication (single-owner per profile)
# -------------------------
...
def setup_or_login():
    ...
    if st.sidebar.button("Erstelle Profil") and new_profile.strip():
        profs = load_json(PROFILES_FILE, {"profiles":[DEFAULT_PROFILE], "active": st.session_state.profile})
        if new_profile in profs.get("profiles", []):
            st.sidebar.error("Profil existiert bereits.")
        else:
            profs.setdefault("profiles", []).append(new_profile)
            profs["active"] = new_profile
    ...
    # switch profile (info)
    profs = load_json(PROFILES_FILE, {"profiles":[DEFAULT_PROFILE], "active": st.session_state.profile})["profiles"]
    sel = st.sidebar.selectbox("Wechsle Profil", profs, index=profs.index(st.session_state.profile) if st.session_state.profile in profs else 0, key="sel_prof")
    if st.sidebar.button("Profil anwenden"):
        if sel != st.session_state.profile:
            # save current
            save_profile_state()
            st.sidebar.info(f"Wechsel zu {sel} — bitte Seite neu laden.")
    # authentication UI
...
def portfolio_snapshot():
    tot_value = 0.0; tot_cost = 0.0; rows = []
...
def calc_volatility(series, window=30):
    if not series or len(series) < window + 1: return 0.0
...
def forecast_linear(series, future_days=30):
    if not series or len(series) < 3: return []
    n = len(series)
...
def risk_analyzer_score():
    ...
    if not snap["rows"]: return {"score":0.0,"vol_comp":0.0,"conc_comp":0.0}
    ...
    return {"score": round(score,2), "vol_comp": round(vol_norm*100,2), "conc_comp": round(top_share*100,2)}
...
def search_portfolio(q):
    q = q.lower()
    return [p for p in st.session_state.portfolio if q in p["name"].lower() or q in p.get("note","").lower() or any(q in t.lower() for t in p.get("tags",[]))]

# -------------------------
# Sidebar: navigation & quick
# -------------------------
...
if page == "Home":
    ...
    try:
        ...
        stats = {"score":0.0,"vol_comp":0.0,"conc_comp":0.0}
    col1,col2,col3 = st.columns(3)
    col1.metric("Positionen", len(snap["rows"]))
    avg_pnl = round(mean([r["pnl"] for r in snap["rows"]]) ,2) if snap["rows"] else 0.0
    ...
    st.write(f"Risikoscore: **{stats['score']}** (Vol:{stats['vol_comp']} / Concentration:{stats['conc_comp']})")
    st.markdown("---")
    ...
    for i in range(5):
        t = f"{rnd.choice(keywords)} News — Änderung {rnd.randint(-4,4)}%"
    ...
    st.title("🏬 Marktplatz")
    st.markdown("Wähle Assets, sieh simulierte Preise und füge Positionen hinzu.")
    categories = {}
    for a in ASSETS:
            ...
            with st.container():
                ...
                with cols[0]:
                    ...
                    if PLOTLY_AVAILABLE:
                        ...
                        fig.update_layout(margin=dict(l=0,r=0,t=20,b=0), height=260, paper_bgcolor=THEMES[st.session_state.settings.get("theme","Dark Black")]["bg"], plot_bgcolor=THEMES[st.session_state.settings.get("theme","Dark Black")]["bg"], font_color=THEMES[st.session_state.settings.get("theme","Dark Black")]["fg"])
                        st.plotly_chart(fig, use_container_width=True)
                    else:
                        st.line_chart({p["date"]:p["price"] for p in series})
                with cols[1]:
                    cur = series[-1]["price"]; sma20 = series[-1]["sma20"]; sma50 = series[-1]["sma50"]
                    ...
                    with st.form(key=f"af_{a['id']}", clear_on_submit=False):
    ...
    if not snap["rows"]:
            ...
            with st.expander(f"{item['name']} — Wert: {r['value']:.2f} €  •  PnL: {r['pnl']:+.2f} € ({r['pnl_pct']:+.2f}%)"):
                ...
                with cols[0]:
                    ...
                    if PLOTLY_AVAILABLE:
                        ...
                        fig.update_layout(margin=dict(l=0,r=0,t=20,b=0), height=300, paper_bgcolor=THEMES[st.session_state.settings.get("theme","Dark Black")]["bg"], plot_bgcolor=THEMES[st.session_state.settings.get("theme","Dark Black")]["bg"], font_color=THEMES[st.session_state.settings.get("theme","Dark Black")]["fg"])
                        st.plotly_chart(fig, use_container_width=True)
                    else:
                        st.line_chart({p["date"]:p["price"] for p in series})
                with cols[1]:
                    div = compute_dividends(item, years=1) if st.session_state.settings.get("dividends_enabled", True) else 0.0
                    st.write(f"Dividenden (1y): {div:.2f} €")
        ...
        st.write(f"Risikoscore: **{ra['score']}** (Vol:{ra['vol_comp']} / Conc:{ra['conc_comp']})")
        ...
        if hist and bench:
            if PLOTLY_AVAILABLE:
                ...
                # scale benchmark to match starting portfolio value
                scale = hist[0]["value"] / bench[0]["price"] if bench[0]["price"]>0 else 1.0
                fig.add_trace(go.Scatter(x=[b["date"] for b in bench], y=[b["price"]*scale for b in bench], name="Benchmark (scaled)"))
                fig.update_layout(margin=dict(l=0,r=0,t=20,b=0), height=320, paper_bgcolor=THEMES[st.session_state.settings.get("theme","Dark Black")]["bg"], plot_bgcolor=THEMES[st.session_state.settings.get("theme","Dark Black")]["bg"], font_color=THEMES[st.session_state.settings.get("theme","Dark Black")]["fg"])
                st.plotly_chart(fig, use_container_width=True)
            else:
    ...
    st.markdown("Gib Zielallokation ein (Summe = 1.0).")
    c1,c2,c3 = st.columns(3)
    tgt_etf = c1.number_input("ETF", min_value=0.0, max_value=1.0, value=0.5, step=0.05, key="t_re_etf")
    tgt_act = c2.number_input("Aktie", min_value=0.0, max_value=1.0, value=0.3, step=0.05, key="t_re_act")
    tgt_crp = c3.number_input("Krypto", min_value=0.0, max_value=1.0, value=0.2, step=0.05, key="t_re_crp")
    ...
    if abs(total - 1.0) > 1e-6:
        ...
        # compute current allocation robustly
        snap = portfolio_snapshot()
        ...
        for k,v in cur_alloc.items(): st.write(f"- {k}: {v*100:.2f}%")
        st.markdown("**Empfehlung (positiv = kaufen / negativ = verkaufen)**")
        for k,v in advice.items(): st.write(f"- {k}: {(v*100):+.2f}%")

elif page == "Simulation":
    st.title("🔬 Simulation & Forecast")
    st.markdown("Simuliere historische Portfolio-Performance (simuliert) und einfache lineare Forecasts.")
    days = st.selectbox("Zeitraum (Tage)", [90, 180, 365, 365*2], index=2)
    hist = simulate_over_time(days)
    if hist:
        if PLOTLY_AVAILABLE:
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=[p["date"] for p in hist], y=[p["value"] for p in hist], name="Portfolio"))
            fig.update_layout(margin=dict(l=0,r=0,t=20,b=0), height=350, paper_bgcolor=THEMES[st.session_state.settings.get("theme","Dark Black")]["bg"], plot_bgcolor=THEMES[st.session_state.settings.get("theme","Dark Black")]["bg"], font_color=THEMES[st.session_state.settings.get("theme","Dark Black")]["fg"])
            st.plotly_chart(fig, use_container_width=True)
        else:
            st.line_chart({p["date"]:p["value"] for p in hist})
        # forecast from last 120 points (if available)
        ...
        if preds:
            if PLOTLY_AVAILABLE:
                ...
                fig2.update_layout(margin=dict(l=0,r=0,t=20,b=0), height=300, paper_bgcolor=THEMES[st.session_state.settings.get("theme","Dark Black")]["bg"], plot_bgcolor=THEMES[st.session_state.settings.get("theme","Dark Black")]["bg"], font_color=THEMES[st.session_state.settings.get("theme","Dark Black")]["fg"])
                st.plotly_chart(fig2, use_container_width=True)
            else:
                st.line_chart({p["date"]:p["price"] for p in ser})
                st.line_chart({p["date"]:p["price"] for p in preds})
    else:
        st.info("Portfolio leer — Simulation nicht möglich.")
    st.markdown("---")
    ...
    stats_data = portfolio_stats()
    if not stats_data:
        st.info("Portfolio leer — keine Statistiken.")
    else:
        st.write(f"Positionen: {stats_data['count']}")
        st.write(f"Durchschn. PnL: {stats_data['avg_pnl']:+.2f} €")
        st.write(f"Volatilität (Snapshot): {stats_data['volatility']:.4f}")
        st.markdown("---")
        snap = portfolio_snapshot()
        cat_vals = {}
        for r in snap["rows"]:
    ...
    guides = load_json(os.path.join(APP_FOLDER, "guides.json"), {
        "ETF_vs_Aktie":"ETFs bündeln viele Aktien — Diversifikation einfach erklärt.",
        "Volatilitaet":"Volatilität = Schwankungsbreite der Renditen.",
        "Rebalancing":"Rebalancing stellt Zielallokation wieder her."
    })
    st.write("Offline-Guides")
    for k,v in guides.items():
        with st.expander(k.replace("_"," ")):
            st.write(v)
            # example graph for each guide (colorful)
            sample = generate_series("GUIDE_"+k, 90, start_price=50.0)
            if PLOTLY_AVAILABLE:
                fig = go.Figure()
                fig.add_trace(go.Scatter(x=[p["date"] for p in sample], y=[p["price"] for p in sample], mode="lines", name="Price"))
                fig.add_trace(go.Scatter(x=[p["date"] for p in sample], y=[p["sma20"] for p in sample], name="SMA20"))
                fig.update_layout(margin=dict(l=0,r=0,t=20,b=0), height=240, paper_bgcolor=THEMES[st.session_state.settings.get("theme","Dark Black")]["bg"], plot_bgcolor=THEMES[st.session_state.settings.get("theme","Dark Black")]["bg"], font_color=THEMES[st.session_state.settings.get("theme","Dark Black")]["fg"])
                st.plotly_chart(fig, use_container_width=True)
            else:
    ...
    st.markdown("Backup & Wiederherstellung deines Profils")
    ...
    if uploaded:
        try:
            ...
            st.session_state.settings = obj.get("settings", st.session_state.settings)
            st.session_state.portfolio = obj.get("portfolio", st.session_state.portfolio)
            st.session_state.history = obj.get("history", st.session_state.history)
            st.session_state.notifications = obj.get("notifications", st.session_state.notifications)
            save_profile_state()
    ...
    st.subheader("Theme / UI")
    theme_choice = st.selectbox("Wähle Theme", list(THEMES.keys()), index=list(THEMES.keys()).index(st.session_state.settings.get("theme","Dark Black")))
    if st.button("Theme anwenden"):
        st.session_state.settings["theme"] = theme_choice
        apply_theme_css(theme_choice)
        save_profile_state()
        st.success("Theme angewendet.")
    st.markdown("---")
    ...
    snapshot_enable = st.checkbox("Tägliche Snapshots aktivieren (manuell speichern weiterhin möglich)", value=st.session_state.settings.get("daily_snapshot_enabled", False))
    ...
    auth = st.session_state.settings.get("auth")
    if not auth:
        st.warning("Kein Passwort gesetzt — setze eines auf Login/Setup.")
    else:
...
# final footer & save
st.markdown("---")
st.markdown("<div class='small'>Offline • Lokale JSONs • Optional: plotly/pandas/reportlab für mehr Features</div>", unsafe_allow_html=True)
save_profile_state()
